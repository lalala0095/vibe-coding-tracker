// A byte-for-byte copy of `front/src/types.ts`. Copied whole rather than
// trimmed to what the phone uses: these are erased at runtime so the unused
// half costs nothing, and a partial copy is exactly how a wire contract starts
// to drift. If the web's copy changes, replace this file rather than patching
// it — `md5sum front/src/types.ts mobile/src/types.ts` should always agree
// apart from this header.
//
// `src/api.ts` is the opposite: it carries only the calls this app makes, so
// unused endpoints cannot rot here unnoticed.
