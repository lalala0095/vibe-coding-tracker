// ─────────────────────────────────────────────────────────────────────────────
// COPY of front/src/lib/week.ts. Everything below this header is byte-identical:
//
//   diff <(tail -n +14 src/lib/week.ts) ../front/src/lib/week.ts
//
// Copied, not ported. Which week an hour lands in decides which week it is paid
// in, and two hand-written versions of that arithmetic would disagree sooner or
// later — the same call `money.ts` and `types.ts` already make in this app.
//
// Note: `sgt.ts` also exports `todaySgt()`, with identical behaviour. Neither
// can drift from the other (Singapore's offset is a constant), but reach for
// `sgt.ts` for timestamp work and this file only for week arithmetic.
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
